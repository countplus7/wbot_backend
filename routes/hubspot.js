...
// OAuth Integration
router.get('/auth/:businessId', authMiddleware, validate([validationSets.commonValidations.businessId]), asyncHandler(async (req, res) => {
...
router.get('/callback', asyncHandler(async (req, res) => {
  ...
  if (!code || !state) {
    return res.status(400).json(createResponse(false, null, 'Missing authorization code or state', null, 'VALIDATION_ERROR'));
  }
...
// CRM Operations
router.post('/contacts/:businessId', authMiddleware, validate([validationSets.commonValidations.businessId]), asyncHandler(async (req, res) => {
  const { businessId } = req.params;
...
router.post('/companies/:businessId', authMiddleware, validate([validationSets.commonValidations.businessId]), asyncHandler(async (req, res) => {
  const { businessId } = req.params;
...
router.post('/deals/:businessId', authMiddleware, validate([validationSets.commonValidations.businessId]), asyncHandler(async (req, res) => {
  const { businessId } = req.params;
...
router.post('/contacts/search/:businessId', authMiddleware, validate([validationSets.commonValidations.businessId]), asyncHandler(async (req, res) => {
  const { businessId } = req.params;
