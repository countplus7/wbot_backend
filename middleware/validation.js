...
const commonValidations = {
  ...
  description: body('description').optional().trim().isLength({ max: 500 }).withMessage('Description must not exceed 500 characters'),
  ...
  endTime: body('endTime').isISO8601().withMessage('End time must be a valid ISO 8601 date')
    .custom((endTime, { req }) => {
  ...
  timeZone: body('timeZone').optional().isLength({ min: 3, max: 50 }).withMessage('Time zone must be between 3 and 50 characters'),
  attendees: body('attendees').optional().isArray().withMessage('Attendees must be an array')
    .custom((attendees) => {
  ...
  // Email validations
  subject: body('subject').trim().isLength({ min: 1, max: 200 }).withMessage('Subject must be between 1 and 200 characters'),
  ...
  toneName: body('tone_name').trim().isLength({ min: 1, max: 50 }).withMessage('Tone name must be between 1 and 50 characters'),
  toneInstructions: body('tone_instructions').trim().isLength({ min: 10, max: 1000 }).withMessage('Tone instructions must be between 10 and 1000 characters'),
  isDefault: body('is_default').optional().isBoolean().withMessage('is_default must be a boolean'),
};
...
const validationSets = {
  ...
  createBusinessTone: [
    commonValidations.businessId,
    commonValidations.toneName,
    commonValidations.toneInstructions,
    commonValidations.isDefault
  ],
  
  updateBusinessTone: [
    param('toneId').isInt({ min: 1 }).withMessage('Tone ID must be a positive integer'),
    body('tone_name').optional().trim().isLength({ min: 1, max: 50 }).withMessage('Tone name must be between 1 and 50 characters'),
    body('tone_instructions').optional().trim().isLength({ min: 10, max: 1000 }).withMessage('Tone instructions must be between 10 and 1000 characters'),
